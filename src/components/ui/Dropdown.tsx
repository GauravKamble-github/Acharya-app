...
export function Dropdown({
  ...
  return (
    <div className={`relative min-w-0 ${fullWidth ? "w-full" : "w-fit max-w-full"} ${className}`}>
      <button
        ref={btnRef}
        type="button"
        ...
        onClick={() => setOpen((v) => !v)}
        onKeyDown={onKeyDown}
        className={`${sizeCls} w-full min-w-0 bg-cream hover:bg-sage text-ink font-medium border border-line inline-flex items-center gap-2 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-forest/40 ${open ? "ring-2 ring-forest/40 bg-sage" : ""}`}
      >
