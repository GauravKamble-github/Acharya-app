...
import { dbConfigured, dbForSlug, isMissingDbObject, type AcharyaSlug } from "./supabase";
import { tableFor } from "./acharya-data";
...
function writeEntry(entry: AICallLog) {
  ...
  if (dbConfigured) {
    ...
    const slug = entry.acharyaSlug || "farmer";
    ...
    try {
      after(async () => {
        try {
          ...
          const { error } = await dbForSlug(slug).from(tableFor(slug, "ai_usage")).insert(row);
      ...
      buildRow()
        .then((row) => dbForSlug(slug).from(tableFor(slug, "ai_usage")).insert(row))
        .then((r) => {
          const e = (r && "error" in r) ? r.error : null;
          if (e && !isMissingDbObject(e)) console.error("[ai-logger] supabase insert failed:", e.message);
        })
