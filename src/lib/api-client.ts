...
import { SUPPORTED_SLUGS, getDefaultAcharya } from "./acharya-config";
import type { ClientAcharyaSlug } from "./acharya-client";

type JsonOk<T> = T;
type JsonErr = { error: string; detail?: string };

export type { ClientAcharyaSlug };
...
export function withAcharyaRoute(path: string): string {
  if (path.startsWith("http")) return path;
  const slug = currentAcharyaSlug();
  const clean = path.startsWith("/") ? path : `/${path}`;
  if ((SUPPORTED_SLUGS as readonly string[]).includes(clean.split("/").filter(Boolean)[0] || "")) {
    return clean;
  }
...
const learner = {
  ...
  event: async (
    ...
    try {
      ...
      // Silently ignore — event tracking failures must never crash the app
    }
  },
};
...
const ai = {
  ...
  chatStream: async function* (payload: {
    ...
    const res = await fetch(withAcharyaPath("/api/chat"), {
      method: "POST",
      credentials: "same-origin",
  ...
  tts: async (text: string, lang: Lang): Promise<Blob> => {
    const learnerId = await currentLearnerId();
    const res = await fetch(withAcharyaPath("/api/tts"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "same-origin",
