#!/usr/bin/env node
// Arjun Acharya — proper schema setup generator
// Creates SQL compatible with the current API (same schema as vajra/taksha)
//
// Usage: node scripts/generate-arjun-setup.js
// Then run the emitted SQL in your Supabase SQL editor.
...
// ============================================================
// ENGLISH CONTENT MAP for M02-M20
// ============================================================
...
function emit() {
  const lines = [];
  lines.push('-- ============================================================================');
  lines.push('-- Arjun Acharya — Complete Production Schema (matches vajra/taksha format)');
  lines.push('-- Run in the DEFAULT Supabase project SQL editor');
  lines.push('-- ============================================================================');
  lines.push('');
  ...
  // Helper function
  lines.push(`CREATE OR REPLACE FUNCTION public.arjun_update_updated_at()`);
  ...
  // Modules
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_modules (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  slug text NOT NULL UNIQUE,`);
  lines.push(`  title_en text NOT NULL,`);
  lines.push(`  title_hi text,`);
  lines.push(`  title_bn text,`);
  lines.push(`  icon text NOT NULL DEFAULT 'book',`);
  lines.push(`  theory_hours numeric NOT NULL DEFAULT 1,`);
  lines.push(`  practical_hours numeric NOT NULL DEFAULT 1,`);
  lines.push(`  sort_order int NOT NULL DEFAULT 0,`);
  lines.push(`  group_key text NOT NULL DEFAULT 'core',`);
  lines.push(`  group_label_en text,`);
  lines.push(`  group_label_hi text,`);
  lines.push(`  group_label_bn text,`);
  lines.push(`  status text NOT NULL DEFAULT 'published' CHECK (status IN ('draft', 'review', 'published')),`);
  lines.push(`  is_deleted boolean NOT NULL DEFAULT false,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  updated_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push(`CREATE TRIGGER arjun_modules_updated_at BEFORE UPDATE ON public.arjun_modules FOR EACH ROW EXECUTE FUNCTION public.arjun_update_updated_at();`);
  lines.push('');

  // Sections
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_sections (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  module_id uuid NOT NULL REFERENCES public.arjun_modules(id) ON DELETE CASCADE,`);
  lines.push(`  slug text,`);
  lines.push(`  title_en text NOT NULL,`);
  lines.push(`  title_hi text,`);
  lines.push(`  title_bn text,`);
  lines.push(`  body_en text,`);
  lines.push(`  body_hi text,`);
  lines.push(`  body_bn text,`);
  lines.push(`  status text NOT NULL DEFAULT 'published' CHECK (status IN ('draft', 'review', 'published')),`);
  lines.push(`  sort_order int NOT NULL DEFAULT 1,`);
  lines.push(`  estimated_hours numeric NOT NULL DEFAULT 1,`);
  lines.push(`  is_deleted boolean NOT NULL DEFAULT false,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  updated_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push(`CREATE TRIGGER arjun_sections_updated_at BEFORE UPDATE ON public.arjun_sections FOR EACH ROW EXECUTE FUNCTION public.arjun_update_updated_at();`);
  lines.push('');

  // Videos
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_videos (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  module_id uuid REFERENCES public.arjun_modules(id) ON DELETE CASCADE,`);
  lines.push(`  youtube_id text NOT NULL,`);
  lines.push(`  title_en text NOT NULL,`);
  lines.push(`  title_hi text,`);
  lines.push(`  title_bn text,`);
  lines.push(`  duration text,`);
  lines.push(`  start_seconds int,`);
  lines.push(`  sort_order int NOT NULL DEFAULT 1,`);
  lines.push(`  is_deleted boolean NOT NULL DEFAULT false,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  updated_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push(`CREATE TRIGGER arjun_videos_updated_at BEFORE UPDATE ON public.arjun_videos FOR EACH ROW EXECUTE FUNCTION public.arjun_update_updated_at();`);
  lines.push('');

  // Users
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_users (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  phone text NOT NULL UNIQUE,`);
  lines.push(`  name text DEFAULT '',`);
  lines.push(`  role text NOT NULL DEFAULT 'learner' CHECK (role IN ('learner', 'admin', 'founder')),`);
  lines.push(`  is_admin boolean NOT NULL DEFAULT false,`);
  lines.push(`  preferred_lang text NOT NULL DEFAULT 'en' CHECK (preferred_lang IN ('en', 'hi', 'bn')),`);
  lines.push(`  is_active boolean NOT NULL DEFAULT true,`);
  lines.push(`  is_deleted boolean NOT NULL DEFAULT false,`);
  lines.push(`  last_seen_on timestamptz,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  updated_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push(`CREATE TRIGGER arjun_users_updated_at BEFORE UPDATE ON public.arjun_users FOR EACH ROW EXECUTE FUNCTION public.arjun_update_updated_at();`);
  lines.push('');

  // Progress
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_progress (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  learner_id uuid NOT NULL REFERENCES public.arjun_users(id) ON DELETE CASCADE,`);
  lines.push(`  module_id uuid NOT NULL REFERENCES public.arjun_modules(id) ON DELETE CASCADE,`);
  lines.push(`  sections_completed text[] NOT NULL DEFAULT '{}',`);
  lines.push(`  completed boolean NOT NULL DEFAULT false,`);
  lines.push(`  completed_at timestamptz,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  updated_at timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  UNIQUE (learner_id, module_id)`);
  lines.push(`);`);
  lines.push(`CREATE TRIGGER arjun_progress_updated_at BEFORE UPDATE ON public.arjun_progress FOR EACH ROW EXECUTE FUNCTION public.arjun_update_updated_at();`);
  lines.push('');

  // Quiz attempts
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_quiz_attempts (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  learner_id uuid NOT NULL REFERENCES public.arjun_users(id) ON DELETE CASCADE,`);
  lines.push(`  module_id uuid REFERENCES public.arjun_modules(id) ON DELETE SET NULL,`);
  lines.push(`  score int NOT NULL,`);
  lines.push(`  total int NOT NULL,`);
  lines.push(`  questions jsonb NOT NULL DEFAULT '[]'::jsonb,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push('');

  // Chat logs
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_chat_logs (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  learner_id uuid REFERENCES public.arjun_users(id) ON DELETE SET NULL,`);
  lines.push(`  module_id uuid REFERENCES public.arjun_modules(id) ON DELETE SET NULL,`);
  lines.push(`  lang text CHECK (lang IN ('en', 'hi', 'bn')),`);
  lines.push(`  user_message text,`);
  lines.push(`  ai_response text,`);
  lines.push(`  response_time_ms int,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push('');

  // Events
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_events (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  learner_id uuid REFERENCES public.arjun_users(id) ON DELETE SET NULL,`);
  lines.push(`  event_type text NOT NULL,`);
  lines.push(`  event_data jsonb,`);
  lines.push(`  created_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push('');

  // Apply logs
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_apply_logs (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  learner_id uuid REFERENCES public.arjun_users(id) ON DELETE SET NULL,`);
  lines.push(`  module_id uuid REFERENCES public.arjun_modules(id) ON DELETE SET NULL,`);
  lines.push(`  log_type text NOT NULL DEFAULT 'self_assessment',`);
  ...
  // AI usage
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_ai_usage (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  lines.push(`  ts timestamptz NOT NULL DEFAULT now(),`);
  lines.push(`  service text NOT NULL,`);
  lines.push(`  model text NOT NULL,`);
  lines.push(`  status text NOT NULL,`);
  lines.push(`  duration_ms int,`);
  lines.push(`  input_tokens int,`);
  lines.push(`  output_tokens int,`);
  lines.push(`  cached_input_tokens int,`);
  lines.push(`  chars int,`);
  lines.push(`  lang text,`);
  lines.push(`  acharya_slug text NOT NULL DEFAULT 'arjun',`);
  lines.push(`  has_image boolean NOT NULL DEFAULT false,`);
  lines.push(`  cost_usd numeric NOT NULL DEFAULT 0,`);
  ...
  // Config
  lines.push(`CREATE TABLE IF NOT EXISTS public.arjun_config (`);
  lines.push(`  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),`);
  ...
  lines.push(`  updated_at timestamptz NOT NULL DEFAULT now()`);
  lines.push(`);`);
  lines.push(`CREATE TRIGGER arjun_config_updated_at BEFORE UPDATE ON public.arjun_config FOR EACH ROW EXECUTE FUNCTION public.arjun_update_updated_at();`);
  lines.push('');

  // Indexes
  lines.push(`CREATE INDEX IF NOT EXISTS arjun_modules_sort_idx ON public.arjun_modules(sort_order);`);
  lines.push(`CREATE INDEX IF NOT EXISTS arjun_sections_module_sort_idx ON public.arjun_sections(module_id, sort_order);`);
  lines.push(`CREATE INDEX IF NOT EXISTS arjun_progress_learner_idx ON public.arjun_progress(learner_id);`);
  lines.push(`CREATE INDEX IF NOT EXISTS arjun_chat_logs_learner_created_idx ON public.arjun_chat_logs(learner_id, created_at DESC);`);
  lines.push(`CREATE INDEX IF NOT EXISTS arjun_events_learner_created_idx ON public.arjun_events(learner_id, created_at DESC);`);
  lines.push(`CREATE INDEX IF NOT EXISTS arjun_users_phone_idx ON public.arjun_users(phone);`);
  lines.push('');

  // RLS
  lines.push(`ALTER TABLE public.arjun_users ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_modules ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_sections ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_videos ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_progress ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_quiz_attempts ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_chat_logs ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_events ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_apply_logs ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_ai_usage ENABLE ROW LEVEL SECURITY;`);
  lines.push(`ALTER TABLE public.arjun_config ENABLE ROW LEVEL SECURITY;`);
  lines.push('');

  // Seed modules
  lines.push('-- ============================================================================');
  lines.push('-- SEED: 21 Arjun Modules');
  lines.push('-- ============================================================================');
  lines.push('');

  MODULES.forEach((m, i) => {
    const gl = m.group_label || { en: m.group_key, bn: m.group_key, hi: m.group_key };
    lines.push(`INSERT INTO public.arjun_modules (slug, title_en, title_hi, title_bn, icon, theory_hours, practical_hours, sort_order, group_key, group_label_en, group_label_hi, group_label_bn, status) VALUES (${q(m.id)}, ${q(m.title.en)}, ${q(m.title.hi)}, ${q(m.title.bn)}, ${q(m.icon)}, 1, 1, ${i}, ${q(m.group_key)}, ${q(gl.en)}, ${q(gl.hi)}, ${q(gl.bn)}, 'published') ON CONFLICT (slug) DO UPDATE SET title_en = EXCLUDED.title_en, sort_order = EXCLUDED.sort_order, updated_at = now();`);
  });
  lines.push('');
  ...
  // Seed sections
  lines.push('-- ============================================================================');
  lines.push('-- SEED: Arjun Sections');
  lines.push('-- ============================================================================');
  lines.push('');
  ...
  MODULES.forEach(m => {
    ...
    secs.forEach((s, si) => {
      const bodyEn = s.content.en || '';
      const bodyHi = s.content.hi || '';
      const bodyBn = s.content.bn || '';
      lines.push(`INSERT INTO public.arjun_sections (module_id, slug, title_en, title_hi, title_bn, body_en, body_hi, body_bn, sort_order, estimated_hours, status)`);
      lines.push(`SELECT m.id, ${q(m.id + '-s' + si)}, ${q(s.title.en)}, ${q(s.title.hi)}, ${q(s.title.bn)}, ${q(bodyEn)}, ${q(bodyHi)}, ${q(bodyBn)}, ${si}, 1, 'published'`);
      lines.push(`FROM public.arjun_modules m WHERE m.slug = ${q(m.id)}`);
      lines.push(`AND NOT EXISTS (SELECT 1 FROM public.arjun_sections s2 WHERE s2.module_id = m.id AND s2.sort_order = ${si});`);
      lines.push('');
    });
  });
  ...
  // Seed videos
  lines.push('-- ============================================================================');
  lines.push('-- SEED: Arjun Videos');
  lines.push('-- ============================================================================');
  lines.push('');

  VIDEOS.forEach((v, i) => {
    lines.push(`INSERT INTO public.arjun_videos (module_id, youtube_id, title_en, title_hi, title_bn, start_seconds, sort_order)`);
    lines.push(`SELECT m.id, ${q(v.youtube_id)}, ${q(v.title.en)}, ${q(v.title.hi)}, ${q(v.title.bn)}, ${v.start_seconds}, ${i}`);
    lines.push(`FROM public.arjun_modules m WHERE m.slug = ${q(v.module_id)}`);
    lines.push(`AND NOT EXISTS (SELECT 1 FROM public.arjun_videos v2 WHERE v2.module_id = m.id AND v2.youtube_id = ${q(v.youtube_id)});`);
    lines.push('');
  });
...
// RUN
// ============================================================
const outPath = path.join(__dirname, 'arjun-setup.sql');
